...
class CoACmix(nn.Module):
    def __init__(self, config, batch_size=1, out_planes=None, head=4, kernel_conv=3, stride=1):

        super(CoACmix, self).__init__()

        self.bert_att= BertCoAttention(config)
        
        #* head 4  注意力的多头数量
        #* k_conv 3  卷积的卷积核尺寸
        #* in_planes 输入维度 4   batch_size*4
        #* out_planes 输出维度 4
        #todo in_plances 目前需要手动改，后期需要改一下

        out_planes = in_planes = batch_size*4
    ...
    def forward(self, context_states, query_states, attention_mask=None):
        
        context_states = torch.unsqueeze(context_states,0) #*  [1,batch_size*4,512,hidden_layers]
        query_states = torch.unsqueeze(query_states,0) #* [1,4,512,768][1,batch_size*4,512,hidden_layers]
       
        q = self.conv1(query_states)   #*  [1,batch_size*4,512,hidden_layers]
        k = v = self.conv2(context_states)  #*  [1,batch_size*4,512,hidden_layers]
        
        b, _, h, w = q.shape

        ## co-att

        context_states = torch.squeeze(context_states, 0)
        query_states = torch.squeeze(query_states, 0)

        #* 使用卷积的结果

        out_att = self.bert_att(context_states,query_states,attention_mask)
        out_att =torch.unsqueeze(out_att,0)

        ## conv
      
        f_all = self.fc(torch.cat([q.view(b,self.head, self.head_dim, h*w), k.view(b,self.head, self.head_dim, h*w), v.view(b,self.head, self.head_dim, h*w)], 1))
        #* [1, 9, 1, 393216]
        f_conv = f_all.permute(0, 2, 1, 3).reshape(1, -1, h, w)  
        #* [1, 9, 512, 768]
...
class BertEERFDHNForMultipleChoice(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the multiple choice classification loss.
            Indices should be in ``[0, ..., num_choices]`` where `num_choices` is the size of the second dimension
    ...
    def __init__(self, config):
        ...
        self.bert_att= BertCoAttention(config)
        self.coattconvmix =CoACmix(config)

        self.init_weights()

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None, inputs_embeds=None, labels=None, pq_end_pos=None, iter=2):
        num_choices = input_ids.shape[1]
        ...
        #* 下面是注意力分数增强词向量的方法
        context_embedding = context_sequence_output.max(1)[0].unsqueeze(1).repeat(1, sequence_output.size(1), 1)
        ending_embedding = query_sequence_output.max(1)[0].unsqueeze(1).repeat(1, sequence_output.size(1), 1)
        #* 上面的维度 [4, 512, 768]
        #* 在这里先进行了最大池化，后进行维度扩充，最后将值复制512份
        #* [4, 512, 768] -> [4,768] -> [4,1,768] -> [4,512,768]
        ...
        #* [4,512], [-1, 1]，求相似度，注意是和完整的序列编码求余弦相似度，范围是[-1,1]
        ...
        # (batch * choice) * 512
        ending_scores = ending_scores * query_attention_mask
        ...
        difference_scores = torch.where(difference_scores == 0, torch.ones_like(difference_scores), difference_scores)
        # (batch * choice) * 512
        ending_scores = (ending_scores - min_ending_scores) * query_attention_mask / difference_scores

        context_sequence_output = context_sequence_output * context_scores.unsqueeze(-1).repeat(1, 1, sequence_output.size(-1))
        query_sequence_output = query_sequence_output * ending_scores.unsqueeze(-1).repeat(1, 1, sequence_output.size(-1))


            #* 下面是并行运行co-attention
        for _ in range(0,iter): #* 在这里 iter的值为 2，也就是进行二遍交互
            #* 第一个参数当作 K和V ,第二个参数当作Q
            cq_biatt_output = self.coattconvmix(context_sequence_output, query_sequence_output, context_attention_mask)
            #* 表示带着问题-选项去阅读短文的细节
            qc_biatt_output = self.coattconvmix(query_sequence_output, context_sequence_output, query_attention_mask)
            #* 表示根据文章去重新审视问题-选项
        ...
        pooled_output=self.dropout(cat_output) #* dropout层的作用就是正规化话，缓解过拟合        
        ...
        #* outputs[2:] 的值 为 ()
        if labels is not None:
            loss_fct = CrossEntropyLoss()   #* 使用的是交叉熵损失函数
            loss = loss_fct(reshaped_logits, labels)
            outputs = (loss,) + outputs
        

        #* 在这里返回的值为 loss 和 reshaped_logits
        return outputs  # (loss), reshaped_logits, (hidden_states), (attentions)
