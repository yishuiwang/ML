...
class PreTrainedTokenizer(object):
    ...
    def encode_plus(self,
        ...
        #* 返回一个编码序列或者序列对和其他信息的字典
        the mask for sequence classification and the overflowing elements if a ``max_length`` is specified.
        #* 如果最大长度被定义好的话，序列分类和元素的掩码会溢出
        Args:
            text: The first sequence to be encoded. This can be a string, a list of strings (tokenized string using
                the `tokenize` method) or a list of integers (tokenized string ids using the `convert_tokens_to_ids`
                method)#*被编码的第一段序列，可以是字符串，可以是字符串列表，可以是数字型列表，在本实验中，其为 短文context
            text_pair: Optional second sequence to be encoded. This can be a string, a list of strings (tokenized
                string using the `tokenize` method) or a list of integers (tokenized string ids using the
                `convert_tokens_to_ids` method)#* 在本实验中，其为问题和单个选项的连接
            add_special_tokens: if set to ``True``, the sequences will be encoded with the special tokens relative
                to their model.#* 如果设置为true的话，则序列会添加和模型相关联的特殊token编码。本实验中，传过来设置为 true
            max_length: if set to a number, will limit the total sequence returned so that it has a maximum length.
                If there are overflowing tokens, those will be added to the returned dictionary#* 最大长度为512位
            stride: if set to a number along with max_length, the overflowing tokens returned will contain some tokens
                from the main sequence returned. The value of this argument defines the number of additional tokens.
            truncation_strategy: string selected in the following options:
                - 'longest_first' (default) Iteratively reduce the inputs sequence until the input is under max_length
                    starting from the longest one at each token (when there is a pair of input sequences)
                #* 默认就是这个，减少输入序列的长度直至其长度小于最大长度，从最长的那个开始
                - 'only_first': Only truncate the first sequence
                - 'only_second': Only truncate the second sequence
                - 'do_not_truncate': Does not truncate (raise an error if the input sequence is longer than max_length)
            pad_to_max_length: if set to True, the returned sequences will be padded according to the model's padding side and
                ...
                - 'right': pads on the right of the sequences   
                Defaults to False: no padding.
            #* 默认为false，如果设置为true的话，则会进行填充，直至满足最大长度
            return_tensors: (optional) can be set to 'tf' or 'pt' to return respectively TensorFlow tf.constant
                or PyTorch torch.Tensor instead of a list of python integers.
            #* 可以被设置为tf或pt的形式
        ...
        def get_input_ids(text):
            if isinstance(text, six.string_types): #* isinstance函数作用是判断一个对象是否是已知的类型
        ...
        first_ids = get_input_ids(text)  #* 对短文context进行编码
        second_ids = get_input_ids(text_pair) if text_pair is not None else None  #* 对问题和单个选项的连接进行编码
        #* 将单词变为数字
        #* 例如：[14, 1253, 101, 18, 4884, 185, 13, 1, 13, 9, 24, 63, 212, 875, 9]
    ...
    def prepare_for_model(self, ids, pair_ids=None, max_length=None, add_special_tokens=True, stride=0,
        ...
        overflowing tokens
        #* 准备一段能被模型使用的输入序列id，给他添加特殊token，在溢出时截断序列
        Args:
        ...
        Return:
            ...
            With the fields:
                ...
                tokens and 1 specifying sequence tokens.
        """
        pair = bool(pair_ids is not None)   #* 其值true
        len_ids = len(ids)   #* 求短文ids的长度
        len_pair_ids = len(pair_ids) if pair else 0  #* 求问题和单个选项连接ids的长度

        encoded_inputs = {}

        # Handle max sequence length
        total_len = len_ids + len_pair_ids + (self.num_added_tokens(pair=pair) if add_special_tokens else 0)
        #* 后面添加的特殊字符的长度为 3 ，这个数字应该是固定的。
        ...
        if max_length and total_len > max_length:
            ids, pair_ids, overflowing_tokens = self.truncate_sequences(ids, pair_ids=pair_ids,
                                                                        ...
                                                                        truncation_strategy=truncation_strategy,
                                                                        stride=stride)
        #* 通过调用上面这个函数，ids中超出512位的部分就被截去了，截去的部分保存在 overflowing_tokens 中

            if return_overflowing_tokens: #* 默认为 false ，即不返回超出的部分
                encoded_inputs["overflowing_tokens"] = overflowing_tokens
                encoded_inputs["num_truncated_tokens"] = total_len - max_length

        # Handle special_tokens
        if add_special_tokens: #* 默认为 true
            sequence = self.build_inputs_with_special_tokens(ids, pair_ids)
            #* 将短文ids 和问题-选项ids 连接起来了
        ...
        if return_special_tokens_mask: #* 默认为 false 即不返回
            encoded_inputs["special_tokens_mask"] = self.get_special_tokens_mask(ids, pair_ids)

        encoded_inputs["input_ids"] = sequence
        if return_token_type_ids: #* 默认为 true
            encoded_inputs["token_type_ids"] = token_type_ids

        #* 经过前面的步骤，下面这两个if语句不会执行
        ...
        #* 不需要补整   pad_to_max_length默认为false      
        ...
        #* 下面这个if语句不执行，pad_to_max_length默认为false 
        ...
        #* 执行下面这个elif语句， return_attention_mask默认为 true
        elif return_attention_mask:
            encoded_inputs["attention_mask"] = [1] * len(encoded_inputs["input_ids"])
            #* 形成一个全为 1 的矩阵

        # Prepare inputs as tensors if asked
        #* return_tensors 的值为 none
        if return_tensors == 'tf' and is_tf_available():
            ...
            logger.warning(
                "Unable to convert output to tensors format {}, PyTorch or TensorFlow is not available.".format(
                    return_tensors))
        #* encoded_inputs是字典的形式
        return encoded_inputs

    def truncate_sequences(self, ids, pair_ids=None, num_tokens_to_remove=0, truncation_strategy='longest_first', stride=0):
        """Truncates a sequence pair in place to the maximum length. #* 截断一个序列直至适应最大长度
            truncation_strategy: string selected in the following options:
                - 'longest_first' (default) Iteratively reduce the inputs sequence until the input is under max_length
                    starting from the longest one at each token (when there is a pair of input sequences).
                ...
                - 'do_not_truncate': Does not truncate (raise an error if the input sequence is longer than max_length)
        """
        if num_tokens_to_remove <= 0:   #* num_tokens_to_remove = 总长度-512
            return ids, pair_ids, []
        ...
        if truncation_strategy == 'longest_first':
            ...
            for _ in range(num_tokens_to_remove):
                if pair_ids is None or len(ids) > len(pair_ids):
                    overflowing_tokens = [ids[-1]] + overflowing_tokens #* 取列表ids的最后一个元素
                    ids = ids[:-1] #* 例如：[14, 1253, 101, 18, 4884] -> [14, 1253, 101, 18]                
                else:
                    pair_ids = pair_ids[:-1]
            window_len = min(len(ids), stride) #* stride = 0
            if window_len > 0:
